...
void PID_Control() {
  ...
  */
  ...
  if (t_now - t_last_PID >= T_sample){      // if the elapsed time is greater than the sampling time, time to send control signal
    t_last_PID = t_now;
    // ================Convert Counter into  Position Angle====================== //

    sensed_output = abs(counter);    
    sensed_angle = (sensed_output / 1200) * 360;          //1200 pluses per 360 degree

 // ===================INSERT Sin Wave Function HERE ================= //
    setpoint_angle = 30 + 15*sin(0.6283*0.001*t);          //y=30+15sin(pi/5 *t), t in seconds
    t = t + T_sample;                                      //increase t by sampling time
  // ==================INSERT CONTROL ALGORITHM HERE ==================== //
    
    error = setpoint_angle - sensed_angle;                  //calculate current error
    total_error += (error + last_error) / 2 * T_sample;     //calculate the cumulated error
    output_Kp = Kp * error;                                //calculate Kp in PID proportional to error
    output_Ki = Ki * total_error;                          //calculate Ki in PID proportional to cumulated error
    output_Kd = Kd * (error - last_error) / T_sample;      //calculate Kd in PID 
